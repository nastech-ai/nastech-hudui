# nastech-hudui v0.8.0

This release turns the HUD into a more operational dashboard: clearer executive status, deeper model analytics, managed Tool Gateway visibility, plugin management, safer update controls, a responsive tab shell, and the official NasTech Teal theme.
...
- **NasTech Teal Theme** — the official Nous NasTech dashboard palette is available from the theme picker.
- **Safer NasTech Update** — `Update nastech` now requires confirmation and shows last-run log/status details.
...
- Fixed NasTech Teal contrast issues on small badges/cards and status text.
- Hardened Gateway update UX so accidental clicks do not run `nastech update`.
