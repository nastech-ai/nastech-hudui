...
class HealthState:
    ...
    config_model: str = ""
    config_provider: str = ""
    nastech_dir_exists: bool = False
    state_db_exists: bool = False
    state_db_size: int = 0
    session_count: int = 0
    last_session_at: Optional[datetime] = None
    nastech_cli_status: str = "unknown"
    nastech_cli_path: str = ""
    nastech_cli_version: str = ""
...
# --- NasTech Agent compatibility baseline -------------------------------------
# The HUD reads ~/.nastech/ and shells out to the `nastech` CLI, so it is coupled
...
VERIFIED_AGENT_VERSION = "0.17.0"  # nastech-agent v0.17.0 (2026.6.19)
...
def _get_dotenv_keys(nastech_dir: str) -> set[str]:
    """Get all key names from nastech .env files."""
    keys: set[str] = set()
    for env_path in [
        os.path.join(nastech_dir, ".env"),
...
def _check_env_key(name: str, nastech_dir: str = "", dotenv_keys: set[str] | None = None) -> bool:
    """Check if a key is set in environment or .env files."""
    if os.environ.get(name, ""):
        return True
    if nastech_dir and dotenv_keys is not None:
...
def _freshness_diag(name: str, path: Path, warning_after: int, broken_after: int) -> DiagnosticStatus:
    ...
    return DiagnosticStatus(
        ...
        age_seconds=age,
        depends_on=[str(path)],
        suggested_fix="Recheck after NasTech writes fresh data." if status != "ok" else "",
        actions=[HealthAction(name="Recheck", kind="refresh")],
    )


def _nastech_cli_info() -> tuple[str, str, str]:
    path = shutil.which("nastech")
    if not path:
        return "broken", "", "nastech CLI not found"
    try:
        result = subprocess.run(
            [path, "--version"],
            capture_output=True,
...
def _schema_diag(
    ...
    if table not in tables:
        return _diag(
            ...
            suggested_fix="Run NasTech once or migrate/restore the state database.",
        )
    ...
    if missing:
        return _diag(
            ...
            "database",
            depends_on=["state.db", table],
            suggested_fix="Update NasTech or migrate/restore state.db so HUD can read the expected schema.",
        )
...
def _layout_diag(nastech_path: Path) -> DiagnosticStatus:
    """Detect pre-0.17 ~/.nastech/ data layout the HUD no longer reads from.
    ...
    for label, old_rel, new_rel in LAYOUT_MIGRATIONS:
        old_path = nastech_path / old_rel
        new_path = nastech_path / new_rel
...
def collect_health(nastech_dir: str | None = None) -> HealthState:
    ...
    # Directory checks
    state.nastech_dir_exists = nastech_path.exists()
    state_db = nastech_path / "state.db"
    ...
    state.readiness.extend([
        _file_diag("NasTech Home", nastech_path, "readiness", "broken"),
        _file_diag("Config", nastech_path / "config.yaml", "readiness", "warning"),
        _file_diag("Environment", nastech_path / ".env", "readiness", "warning"),
        _file_diag("State Database", state_db, "readiness", "broken"),
        _file_diag("Logs", nastech_path / "logs", "readiness", "warning"),
        _file_diag("Model Cache", nastech_path / "models_dev_cache.json", "readiness", "warning"),
        _file_diag("Plugins", nastech_path / "plugins", "readiness", "warning"),
        _layout_diag(nastech_path),
    ])

    cli_status, cli_path, cli_version = _nastech_cli_info()
    state.nastech_cli_status = cli_status
    state.nastech_cli_path = cli_path
    state.nastech_cli_version = cli_version
    state.readiness.append(_diag("NasTech CLI", cli_status, cli_version, "readiness"))

    state.freshness.extend([
        _freshness_diag("state.db", state_db, warning_after=3600, broken_after=86400),
        _freshness_diag("models_dev_cache.json", nastech_path / "models_dev_cache.json", warning_after=86400 * 7, broken_after=86400 * 30),
    ])
    ...
    try:
        ...
        state.database.append(_diag(
            ...
            suggested_fix="Restore state.db from a valid NasTech state database.",
        ))
    ...
    if last_session_at:
        ...
        state.freshness.append(_diag(
            ...
            "freshness",
            depends_on=["sessions"],
            suggested_fix="Start a NasTech session, then recheck health.",
    ...
    # API keys
    dotenv_keys = _get_dotenv_keys(nastech_dir)

    known_names = {key_name for key_name, _, _ in EXPECTED_KEYS}
    for key_name, source, note in EXPECTED_KEYS:
        present = _check_env_key(key_name, nastech_dir, dotenv_keys)
    ...
    state.services.append(
        _check_pid_file("Telegram Gateway", nastech_path / "gateway.pid")
    )
    state.services.append(
        _check_systemd_service("Gateway (systemd)", "nastech-gateway")
    )
    state.services.append(
    ...
    state.features.extend([
        _feature_diag(
            ...
            "requires NasTech CLI, config, sessions, and messages",
            depends_on=["NasTech CLI", "Config", "sessions table", "messages table"],
        ...
        _feature_diag(
            ...
            suggested_fix="Update NasTech or migrate state.db to include model analytics columns.",
            actions=[
        ...
        _feature_diag(
            ...
            warn=not plugins_ok,
            depends_on=["plugins"],
            suggested_fix="Create the NasTech plugins directory or install a plugin.",
        ...
        _feature_diag(
            "Cron",
            cli_status != "broken",
            "uses NasTech CLI for cron actions",
            depends_on=["NasTech CLI"],
            suggested_fix="Install NasTech CLI or fix PATH.",
